var setup = function () {
    ...
    var options = {
        "fields": {
            ...
            "file": {
                "type": "file",
                "label": "Ανεβάστε αρχείο"
            },
            "check": {
                "type": "checkbox",
                "rightLabel": "Επιθυμώ να λάβω το Πιστοποιητικό μέσω email!",
                "label": "Ηλεκτρονική παραλάβή"
            }
        }
    };
    var data = {
        ...
        if (schema) {
            ...
            if (disableErrorHandling) {
                Alpaca.defaultErrorCallback = function (error) {
                    console.log("AdminForms encountered an error while previewing form -> " + error.message);
                };
            }
            else {
    ...
    var types = [{
        ...
        "type": "number",
        "title": "Αριθμός",
        "description": "Αριθμητικό"
    }, {
        "type": "boolean",
        "title": "Boolean",
        "description": "Λογικό (Ναι/Όχι)"
    }, {
        "type": "object",
        "title": "Αντικείμενο",
        "description": "Αντικείμενο"
    }, {
        "type": "array",
        "title": "Πίνακας",
        "description": "Πίνακας"
    ...
    var afterAlpacaInit = function () {
        ...
        for (var type in Alpaca.fieldClassRegistry) {
            ...
            var title = translate_Field_Titles(instance.getTitle());
    ...
    var removeField = function (alpacaId) {
		...
		//#fix (replaced) : parentField.removeItem(alpacaId, function () {
    ...
    var loadForm = function (formId) {
        ...
        $.ajax({
            ...
            data: JSON.stringify({ formId: formId, submittedformId:0 }), //Load Form Template only
            dataType: 'json',
            ...
            success: function (response) {
                try {
                    ...
                    alert("Your form was loaded from remote storage");
                }
                catch (e) {
                    // bad value
                }
    ...
    var saveForm = function (orgId,title,config) {
       
        $.ajax({
            type: 'POST',
            url: webServicesBaseURL + 'FormsService.asmx/SaveForm',
            data: JSON.stringify({ orgId: orgId,title: title , data: config }),
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            success: function (response) {
    ...
    $(".save-button").off().click(function () {
        var frmtitle = $('#frmTitle').val();
        if (frmtitle.length < 5) {
            MessageManager.show('#frmSaveStatus', 'label label-warning', 'Ο Τίτλος της φόρμας δεν είναι σωστός');
            return;
        }
        ...
        if (V_orgid > 0)
        {
            saveForm(V_orgid, frmtitle, configString);
        MessageManager.show('#frmSaveStatus', 'label label-success', 'Αποθηκεύτηκε');
        }
...
var MessageManager = {
    show: function (element,classes,content) {
